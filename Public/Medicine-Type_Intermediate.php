...
if( isset($_GET['id'])) 
    $type_id = intval($_GET['id']); // collecting type id from previous url session
else
    $type_id = 0;

$total_result = $conn->query("SELECT COUNT(*) AS total FROM medicine WHERE type_id = $type_id");
...
<header>
    ...
    <div class="sort-buttons">
        <a href="Medicine-Type_Intermediate.php?id= <?php echo $type_id ?> &sort=m.name_asc" class="<?php echo ($sort === 'm.name_asc') ? 'active' : ''; ?>">Sort A → Z</a>
        <a href="Medicine-Type_Intermediate.php?id= <?php echo $type_id ?> &sort=m.name_desc" class="<?php echo ($sort === 'm.name_desc') ? 'active' : ''; ?>">Sort Z → A</a>
        <a href="Medicine-Type_Intermediate.php?id= <?php echo $type_id ?> &sort=m.medicine_id" class="reset">Reset</a>
    </div>
    ...
    <div class="pagination">
        <?php if ($page  > 1): ?>
            <a href="Medicine-Type_Intermediate.php?id= <?php echo $type_id ?> &page=1&sort=<?php echo $sort ?>">« First</a>
            <a href="Medicine-Type_Intermediate.php?id= <?php echo $type_id ?> &page=<?php echo $page  - 1; ?>&sort=<?php echo $sort; ?>"><</a>
        <?php endif; ?>

        <?php for ($i = 1; $i <= $total_pages; $i++): ?>
            <a href="Medicine-Type_Intermediate.php?id= <?php echo $type_id ?> &sort=<?php echo $sort; ?> &page=<?php echo $i; ?>" class="<?php echo ($i == $page ) ? 'active' : ''; ?>"><?php echo $i; ?></a>
        <?php endfor; ?>

        <?php if ($page  < $total_pages): ?>
            <a href="Medicine-Type_Intermediate.php?id= <?php echo $type_id ?> &page=<?php echo $page  + 1; ?>&sort=<?php echo $sort; ?>">></a>
            <a href="Medicine-Type_Intermediate.php?id= <?php echo $type_id ?> &page=<?php echo $total_pages; ?>&sort=<?php echo $sort ?>">Last »</a>
        <?php endif; ?>
    </div>

    <div class="retreat_button">
        <a href="Medicine_Type_Database.php">Return to Type List</a>
    </div>
