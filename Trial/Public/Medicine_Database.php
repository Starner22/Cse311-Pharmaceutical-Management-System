...
$total_result = $conn->query("SELECT COUNT(*) AS total FROM medicine");
$total_row = $total_result->fetch_assoc();
$total_medicines = $total_row['total'];
$total_pages = ceil($total_medicines / $limit); //dont floor this
...
<header>
    ...
    <div class="sort-buttons">
        <a href="Medicine_Database.php?sort=m.name_asc" class="<?php echo ($sort === 'm.name_asc') ? 'active' : ''; ?>">Sort A → Z</a>
        <a href="Medicine_Database.php?sort=m.name_desc" class="<?php echo ($sort === 'm.name_desc') ? 'active' : ''; ?>">Sort Z → A</a>
        <a href="Medicine_Database.php?sort=m.medicine_id" class="reset">Reset</a>
    </div>
    ...
    <div class="pagination">
        <?php if ($page > 1): ?>
            <a href="Medicine_Database.php?page=1&sort=<?php echo $sort ?>">« First</a>
            <a href="Medicine_Database.php?page=<?php echo $page - 1; ?>&sort=<?php echo $sort; ?>"><</a>
        <?php endif; ?>

        <?php for ($i = 1; $i <= $total_pages; $i++): ?>
            <a href="Medicine_Database.php?sort=<?php echo $sort; ?>&page=<?php echo $i; ?>" class="<?php echo ($i == $page) ? 'active' : ''; ?>"><?php echo $i; ?></a>
        <?php endfor; ?>

        <?php if ($page < $total_pages): ?>
            <a href="Medicine_Database.php?page=<?php echo $page + 1; ?>&sort=<?php echo $sort; ?>">></a>
            <a href="Medicine_Database.php?page=<?php echo $total_pages; ?>&sort=<?php echo $sort ?>">Last »</a>
        <?php endif; ?>
</div>
